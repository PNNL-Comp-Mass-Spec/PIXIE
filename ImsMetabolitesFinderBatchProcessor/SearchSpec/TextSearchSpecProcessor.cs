...
namespace ImsMetabolitesFinderBatchProcessor.SearchSpec
{
    ...
    public class TextSearchSpecProcessor
    {
        ...
        /// </returns>
        private IEnumerable<ImsInformedProcess> ProcessLine(string utility, string line, int lineNumber, List<Exception> exceptions, ref bool firstLine, ref int id)
        {
            bool outputWhereInputsAre = string.IsNullOrEmpty(this.outputPath);
            string datasetName = string.Empty;
            IList<ImsInformedProcess> imsInformedProcesses = new List<ImsInformedProcess>();
            ...
            try
            {
                ...
                IEnumerable<string> ionizationList;
                if (parts.Count() <= 2)
                {
                    ionizationList = this.InferIonization(datasetName);
                }
                else
                {
                    ionizationList = parts[2].Split(":,&".ToCharArray(), StringSplitOptions.RemoveEmptyEntries);
                }
                ...
                foreach (string ionization in ionizationList)
                {
                    ...
                    id++;
                    imsInformedProcesses.Add(process);
                }
                
                return imsInformedProcesses;
            }
            catch (Exception e)
            {
        ...
        /// </exception>
        private IEnumerable<string> InferIonization(string datasetName)
        {
            IList<String> ionizationList = new List<string>();
            if (datasetName.Contains("pos"))
            {
                ionizationList.Add("M+H");
                ionizationList.Add("M+Na");
            }
